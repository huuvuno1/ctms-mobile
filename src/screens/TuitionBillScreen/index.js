...
const TuitionBillScreen = ({ navigation }) => {
  ...
  const [startDay, setStartDay] = useState(new Date().setFullYear(new Date().getFullYear() - 1));
  ...
  return (
    <View style={styles.wrapper}>
      <DatePicker onChange={setStartDay} label="Từ ngày" shiftValue={30}
      ...
      <ScrollView style={styles.wrapSchedule}>
        <Text style={[styles.scheduleDay, {
          ...
          marginTop: 20,
          textAlign: 'center'
        }]}>Tìm thấy {data.length} hóa đơn</Text>
        {data?.length ? data?.map((item, index) => (
          <React.Fragment key={`card-${index}`}>
            <View
              ...
              <Text style={styles.scheduleDay}>Ngày lập: {item.createdAt}</Text>
            ...
            >
              <View style={styles.table}>
                {fields.map((field, index) => (index === 4 && item[field.key] === '0,0') ? undefined : (
                  <View style={styles.row} key={index}>
                    <Text style={[styles.cell, styles.cellW1]}>{field.name}</Text>
                    {(() => {
                      switch (field) {
                        case fields[6]:
                          return item['owed'] === '0,0' ?
                            <View style={[styles.pay]}>
                              <Text style={styles.billPaid}>Đã thanh toán</Text>
                            </View> : <View style={[styles.pay]}>
                              <Text style={styles.billPaid}>Còn nợ</Text>
                            </View>
                        case fields[5]:
                          return (<TouchableOpacity style={[styles.cell, styles.cellW2, {
                            alignItems: 'center'
                          }]}>
                            <Image style={styles.into} source={into} />
                          </TouchableOpacity>)
                        default:
                          return <Text style={[styles.cell, styles.cellW2, {
                            textAlign: 'center'
                          }]}>{item[field.key]}</Text>
